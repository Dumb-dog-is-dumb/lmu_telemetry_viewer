...
onCursorMove((tSec) => {
  if (tSec == null || currentMapPoints.length === 0) return setMapCursor(null);
  setMapCursor(nearestPointAtTime(currentMapPoints, tSec));
});
...
export async function loadMap(file, startTs, endTs) {
  const [lonRows, latRows] = await Promise.all([
    getChannel(file, "GPS Longitude", startTs, endTs),
    getChannel(file, "GPS Latitude", startTs, endTs),
  ]);
